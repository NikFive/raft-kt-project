...
class State(
    ...
    suspend fun appendEntries(request: AppendRequest): AppendResponse {
        ...
        if (request.term >= this.term) {
            println("Heartbeat received: term=${request.term}, leaderId=${request.leaderId}, at=${System.currentTimeMillis()}")
            this.term = request.term
            this.votedFor = null
        ...
        if (request.prevLogIndex != -1) {
            ...
            if (!logValid) {
                println("Master log is not cool")
